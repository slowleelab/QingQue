"""实验 2: 检索方法对比

对比 BM25 vs 向量 vs 混合检索，以及 RRF k 参数调优。
需要 bot 服务运行中 (make dev)。
"""

from __future__ import annotations

import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parents[2]))

from scripts.experiments.metrics import hit_at_k, mrr, ndcg_at_k, precision_at_k
from scripts.experiments.runner import (
    BOT_BASE_URL,
    EVAL_DATA_PATH,
    format_results_table,
    load_eval_data,
    run_eval_batch,
    save_report,
)


def main() -> None:
    print("=" * 60)
    print("实验 2: 检索方法对比")
    print("=" * 60)

    eval_data = load_eval_data(EVAL_DATA_PATH)
    print(f"加载评估数据: {len(eval_data)} 条")

    results: list[dict] = []

    # 子实验 2a: 检索方法对比
    print("\n--- 子实验 2a: 检索方法对比 ---")
    for search_type in ["bm25_only", "vector_only", "hybrid"]:
        print(f"\n运行: {search_type}...")
        relevances, avg_latency = run_eval_batch(
            eval_data=eval_data,
            search_type=search_type,
            rerank=True,
            top_k=5,
            base_url=BOT_BASE_URL,
        )

        metrics = {
            "mrr": mrr(relevances),
            "hit@3": hit_at_k(relevances, 3),
            "hit@5": hit_at_k(relevances, 5),
            "ndcg@5": ndcg_at_k(relevances, 5),
            "p@3": precision_at_k(relevances, 3),
            "avg_latency_ms": round(avg_latency, 1),
        }
        print(f"  MRR={metrics['mrr']:.4f}, Hit@3={metrics['hit@3']:.4f}")
        results.append({"params": {"search_type": search_type, "实验": "2a"}, "metrics": metrics})

    # 子实验 2b: RRF k 参数调优
    print("\n--- 子实验 2b: RRF k 参数调优 ---")
    for k in [10, 30, 60, 100]:
        print(f"\n运行: RRF k={k}...")
        relevances, avg_latency = run_eval_batch(
            eval_data=eval_data,
            search_type="hybrid",
            rerank=True,
            rrf_k=k,
            top_k=5,
            base_url=BOT_BASE_URL,
        )

        metrics = {
            "mrr": mrr(relevances),
            "hit@3": hit_at_k(relevances, 3),
            "hit@5": hit_at_k(relevances, 5),
            "ndcg@5": ndcg_at_k(relevances, 5),
            "p@3": precision_at_k(relevances, 3),
            "avg_latency_ms": round(avg_latency, 1),
        }
        print(f"  MRR={metrics['mrr']:.4f}, Hit@3={metrics['hit@3']:.4f}")
        results.append({"params": {"rrf_k": k, "实验": "2b"}, "metrics": metrics})

    # 输出对比表
    table = format_results_table(results)
    print(f"\n{table}")

    # 保存报告
    report_path = save_report("experiment2_retrieval_method", results)
    print(f"\n报告已保存: {report_path}")


if __name__ == "__main__":
    main()
