...
- **SmartCSBotUser**：模拟客户完整对话流程（send → poll）
- 问题池：12 条覆盖常见信用卡咨询意图
...
| 指标 | 目标 | 说明 |
|------|------|------|
| /api/chat/send p50 | < 50ms | 消息入队（Redis 写） |
| /api/chat/poll p50 | < 3s | 含意图分类 + RAG 检索 + LLM 生成 |
| /api/chat/poll p99 | < 10s | 含降级场景 |
| /api/health p50 | < 5ms | 健康检查 |
| 错误率 | < 1% | 超时/异常比例 |
...
4. **连接池**：PostgreSQL/Redis 连接池大小可通过配置调优。
