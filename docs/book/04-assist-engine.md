...
**五阶段流水线** (对应下面这张图, 从下往上读):
- **Phase 1 评估**: 先想清楚"现在是什么情况" — 客户意图、服务期/营销期/风控、紧急程度
- **Phase 2 执行**: 三条线并行干活 — 话术生成、营销推荐 (延迟 500ms, 别抢话)、风控检查
- **Phase 3 决策**: 综合判断"该不该推" — 刚推过话术? 30 秒内别重复打扰
- **Phase 4 仲裁**: 三路结果有冲突时定优先级 — 风控提醒 > 话术 > 营销
- **Phase 5 推送**: 脱敏 + 去重 + 推送到坐席屏幕
