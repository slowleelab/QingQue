...
**阶段 ② 分发 — "按号排队"**
后台有个"叫号员" (Consumer Group, 读作"消费组", 是什么见 3.2.3) 把消息从 Redis 里读出来, 按客户会话分到各自的队列. **每个会话一个队列、一个处理员** — 这是全章最重要的一句话: 同一个客户连发的 10 条消息, 处理顺序和发送顺序完全一致, 不会出现"第 3 条先被回答、第 2 条后到"的乱序事故.
...
### 3.2.3 概念先修:Redis、Stream、消费组到底是什么

> 本节写给**不写代码的读者** (业务、项目经理、测试)。后面的 3.3 / 3.4 会大量使用 Redis、Stream、Worker 这些词, 先把它们长什么样讲清楚, 后面读起来就不会迷路。开发读者可以跳过本节, 直接看 3.3 的实现细节。

**Redis 是什么** — 一句话: 一个"所有人共享的公共储物柜" (内存数据库)。任何一台服务机器都能往里存东西、取东西, 存取都很快 (毫秒级), 但数据放在内存里, 机器重启数据会消失 (所以只放"临时状态", 永久数据都在 PostgreSQL 里)。它和数据库表格的区别: 表格是"存完查询", 储物柜是"放进去、取出来、传个话", 强调的是速度和共享, 不是复杂的检索。

**Stream 是什么** — 一句话: 储物柜里的一叠"按顺序编号的业务单据" (消息队列)。任何服务都能往这叠单据的**末尾**加一张 (写消息), 也能从**开头**按顺序取 (读消息)。一张单据长这样:

```text
单据编号: 1697000000000-0          ← 编号 = 时间戳 + 序号, 保证全系统唯一且有序
单据内容: { 会话ID: "sess-123", 消息: "上个月账单多少钱", 客户ID: "cust-9" }
```

这叠单据有三个规则:
1. **只能末尾追加、按序读取** — 就像银行的排队取号, 号码不会乱;
2. **有堆叠上限 (MAXLEN 10000)** — 超过 1 万张就把最旧的扔掉, 防止储物柜被单据塞爆 (内存有界);
3. **谁都能往里放, 谁都能往外取** — 这是系统能水平扩展 (加机器) 的根基: 3 台机器共用这一叠单据, 而不是每台机器各存各的。

**消费组 (Consumer Group) 是什么** — 一句话: 一组处理员 (3 台机器上的消费进程) 共用的**一本"取单登记本"**。这叠单据由整组处理员共享, 登记本保证:

- **同一张单据只被一个处理员取走** — 不会出现"3 台机器同时处理同一条消息";
- 处理员取走单据后, 必须在登记本上**打勾 (XACK)** 表示"这张办完了";
- 打了勾的单据才算结束; 没打勾的会被登记在"未完成清单 (PEL)"里, 超过 60 秒还没打勾, 说明取单的处理员可能宕机了, 其他处理员可以**接管 (XAUTOCLAIM)** 重新处理;
- 正因为"没打勾就可能被重新投递", 处理端必须**幂等** — 同一张单据重复处理也不能产生两次效果 (好比同一笔刷卡请求重复送达, 银行只扣一次款)。

```mermaid
flowchart LR
    C[客户发消息] -->|XADD 追加单据| S[(单据堆<br/>Redis Stream)]
    S -->|XREADGROUP 取单| G[消费组登记本<br/>同一张只被取一次]
    G --> W1[处理员 1<br/>机器 A]
    G --> W2[处理员 2<br/>机器 B]
    G --> W3[处理员 3<br/>机器 C]
    W1 -->|XACK 打勾| S
...
**这张图怎么读**: 客户消息先变成单据进堆 → 消费组登记本把单据派给某个处理员 (只派一次) → 处理员办完打勾; 谁 60 秒没打勾, 系统认定他挂了, 把单据转给别的处理员。**注意**: 消费组只保证"不丢、不重取", 不保证"同一个客户的单据按顺序处理"——3 台机器并行取单时, 客户连发的 3 条消息可能被 3 个处理员同时取走。顺序问题由 3.4 的 per-session Worker 解决。
...
> 不写代码的读者: 这段代码的作用只有一句 — **在 Redis 里创建"取单登记本"** (如果单据堆还不存在就顺带建一个空堆)。后面的 XREADGROUP / XACK / XAUTOCLAIM 都是在这个登记本上操作。术语对照见本章末尾的"3.13 黑话速查表"。
...
**"Worker"到底是什么 — 写给不写代码的读者**: Worker 不是一台机器, 也不是一个独立程序, 而是**运行在 Bot 服务进程里的一个"工作台"**。一个进程里可以同时开几千个工作台 (技术上叫**协程**, 你可以把它理解成"一个能随时暂停/恢复的小任务", 比开一个真线程轻量得多), 每个工作台只认领一个会话:

- 工作台手边有一个**收件盒** (`asyncio.Queue`), 只收自己那个客户的新消息, 先进先出;
- 工作台从收件盒取一条 → 交给 AI 处理 → 处理完在单据堆登记本上**打勾 (XACK)** → 再取下一条;
- 收件盒空等 300 秒 (5 分钟) → 工作台自动下班 (释放资源); 客户再发消息时, 系统现场再开一个工作台 (自动创建)。

**为什么不是"3 台机器抢着处理"?** 上一节 3.2.3 说过, 消费组只保证"不重取", 不保证"按顺序" — 客户连发 3 条, 3 台机器可能同时取走, 回答乱序。有了专属工作台: 同一个客户的 3 条消息**只进这一个收件盒**, 永远按发送顺序一条一条处理; 而不同客户的消息仍然由各自的工作台并行处理, 互不阻塞。这就是"**同一个客户串行, 不同客户并行**"——银行客服的硬性时序要求: 客户必须觉得"我的话一条接一条被回复", 而不是"三条消息三个 AI 抢着回"。

`router.py:289-340` `_session_worker` 就是上面说的"工作台"的实现, 每个 session_id 独占一个 Worker:

```python
# 简化版 — 不写代码的读者可以跳过, 记住三句话即可:
# ① 从收件盒取一条; ② 处理 + 打勾; ③ 5 分钟没消息就下班
...
**"Semaphore 满荷"是什么意思?** Semaphore (信号量) 是系统的一个"**全局同时处理上限**": 同一时刻最多 10 条消息在真正调用 AI, 防止高峰把所有机器打满。上限用完了就叫"满荷"。**满荷 ≠ 拒绝客户** — 系统不排队死等, 而是立刻用预先写好的话术先回应 (挂失 → "正在优先处理", 账单 → "稍后返回"), 让客户先得到"回应"而不是"排队", 5 秒冷却期内同一会话不重复兜底, 避免连续发模板话术。

满荷时走 `<5ms` 正则快速兜底话术 (`_FAST_REPLIES`). RuleLoader 返回的是
...
**业务背景**: 消息到了 Worker, 现在要"真正思考"了. 但"思考"不等于"什么都交给大模型" — 银行场景里, 有些话不能乱说 (挂失要转人工), 有些事不能乱做 (调额要确认), 有些问题不能乱答 (知识要查库). 所以 `LumioAgent` 是一个**规则优先的决策器**: 能用规则判断的绝不让模型自由发挥, 模型只负责"分类"和"生成"两件最需要它的事. 下文代码块写给开发读者; 业务/项目经理/测试读者看上面的 6 步表格 + 每步的"关键设计"即可, 不读代码也不影响理解。
...
## 3.12 黑话速查表 (业务 / 项目经理 / 测试对照)

> 本章所有技术名词的"一句话人话解释"。开发读者可跳过; 其他读者读到不懂的词回来查这一节即可。更完整的跨章术语见 [附录 A 术语表](appendix/A-glossary.md)。

| 术语 | 一句话解释 | 章节 |
|---|---|---|
| Redis | 所有服务共享的"公共储物柜" (内存数据库), 存取毫秒级, 重启丢失 (只放临时状态) | 3.2.3 |
| Stream | 储物柜里一叠"按顺序编号的业务单据" (消息队列), 末尾追加、按序读取、上限 1 万条 | 3.2.3 |
| 消费组 (Consumer Group) | 一组处理员共用的"取单登记本": 同一张单据只被一个处理员取走 | 3.2.3 / 3.3.1 |
| XADD | 往单据堆末尾放一张新单据 (写消息) | 3.2 |
| XREADGROUP | 处理员从单据堆按顺序取一张 (登记本保证不重取) | 3.3 |
| XACK | 处理员打勾"这张办完了" | 3.3.2 / 3.4 |
| PEL | 登记本上"取走但没打勾"的未完成清单 | 3.3.2 |
| XAUTOCLAIM | 单据 60 秒没人打勾 → 判定原处理员宕机, 其他处理员接管重处理 | 3.3.2 |
| MAXLEN | 单据堆上限 (1 万条), 超过扔最旧的 (内存有界) | 3.3.3 |
| at-least-once | "至少一次": 保证消息不丢, 但可能重复投递 | 3.3.2 |
| 幂等 | 同一请求重复处理也只产生一次效果 (像同一笔刷卡重复送达只扣一次款) | 3.2.1 / 3.3.2 |
| TTL | 数据的有效期, 到期自动消失 (如结果缓存 2 分钟、幂等键 5 分钟) | 3.1 / 3.2.1 |
| per-session Worker | 每个会话专属的"工作台" (一个协程): 同客户消息严格按发送顺序处理, 不同客户并行 | 3.4 |
| 协程 | 轻量级小任务 (工作台), 一个进程能同时开几千个, 比线程便宜得多 | 3.4 |
| asyncio.Queue | 工作台手边的"收件盒", 先进先出 | 3.4 |
| Semaphore (信号量) | 系统全局"同时处理中"的上限 (默认 10), 用完了叫"满荷" | 3.4.2 / 3.7 |
| 快速兜底 | 满荷时不排队死等, 用预先写好的话术先回应客户 | 3.4.2 |
| 冷却期 | 同一会话 5 秒内不重复给快速兜底话术 (避免连续模板话) | 3.4.2 |
| 长轮询 | 客户端问"好了吗", 服务器"等有结果再答" (最长等 30 秒) | 3.1 |
| Pub/Sub | 广播: 结果好了"喊一声", 在听的人立刻应答 | 3.1 |
| 死信队列 | 重试 3 次仍失败的消息集中地, 管理端可查看/重放 (不无声丢失) | 3.3.2 / 第 8 章 |
| LLM | 大语言模型, 只负责"判断意图"和"组织回答"两件事 | 3.5 |
| 意图 (intent) | 模型判断"客户想干嘛": 查账单/挂失/投诉/闲聊 | 3.5 |
| 路由 | 按意图把消息分派到对应流程: 查知识 / 办业务 / 调工具 / 闲聊 | 3.5 |
| RAG | 先检索知识库相关条文, 再让模型结合条文回答 (不凭空编) | 3.5.1 / 第 5 章 |
| MCP | 调用银行工具的统一通道 (查账单/提额等 22 个工具) | 3.5.3 / 第 7 章 |
| pending_action | 待客户确认的敏感操作 (存 Redis 跨轮不丢, 5 分钟过期) | 3.5.4 / 第 17 章 |
| 槽位 (slot) | 办业务需要收集的信息项 (分期金额、期数、卡号后四位…) | 3.6 |
| 实体 (entity) | 从客户话里抽出的关键信息 (卡号、金额、日期) | 3.6 |
| ZSET | 带分数的有序清单 (这里记每个客户当前活跃的会话数) | 3.2.2 |
| 客户画像 | 从历史对话推断的客户标签 (卡种/VIP 等级/风险偏好) | 3.9 / 第 16 章 |
| 降级 | 模型/检索不可用时用次优内容回应 (HTTP 仍 200, 客户无感) | 3.7 / 第 8 章 |
| 熔断 | 上游连续失败 N 次后自动"断开"不再调用, 定期探测恢复 | 3.7 / 第 4 章 |
| WebSocket | 常驻连接, 服务器可随时主动推送 (长轮询的升级版, 增强项) | 3.1 / ws_router |
