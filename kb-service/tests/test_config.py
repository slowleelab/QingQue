...
def test_settings_defaults():
    """测试默认配置"""
    settings = Settings()
    assert settings.service_name == "kb-service"
    assert settings.database.host == "localhost"
    assert settings.database.port == 5433
    assert settings.elasticsearch.index_prefix == "kb"
    assert settings.elasticsearch.chunks_index == "kb_kb_chunks"
    assert settings.kafka.ingest_topic == "kb.ingest.request"
    assert settings.kafka.dlq_topic == "kb.ingest.dlq"
