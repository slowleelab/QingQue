...
基于 Spring Boot 3.1、Netty 4.1 和 ZooKeeper 3.8 实现的星型拓扑长连接微服务系统（Lumio 灵智客服的子工程）。
...
```
chat-svc/
├── chat-common/                          # 公共模块（Maven artifactId）
│   └── model/                       # 消息定义（Message, Session, Agent, ChatMessage）
│
├── chat-transport/                       # 传输模块（聚合 pom）
│   ├── chat-transport-core/              # 核心抽象
...
│   ├── chat-transport-netty/             # Netty实现
│   ├── chat-transport-zookeeper/         # ZK集成
│   └── chat-transport-spring-boot-starter/  # Spring Boot自动配置
│
├── chat-customer-server/                 # 客户服务
│   ├── netty/                       # Netty服务器
...
```bash
# 节点1
java -jar chat-customer-server/target/chat-customer-server-1.0.0.jar \
  --server.port=8080 \
...
```bash
# 节点1
java -jar chat-agent-server/target/chat-agent-server-1.0.0.jar \
  --server.port=8081 \
...
```xml
<dependency>
    <groupId>com.lumio.chatsvc</groupId>
    <artifactId>chat-transport-spring-boot-starter</artifactId>
